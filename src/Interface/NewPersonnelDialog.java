...
public class NewPersonnelDialog extends JDialog{
		...
		public NewPersonnelDialog(Frame owner, boolean modal){
			...
			this.valide = new JButton("Valid�");
			this.valide.addActionListener(e -> {
				this.createPersonnel();
			});
			this.annulle = new JButton("Annul�");
		...
		private void createPersonnel() {
			...
			if(nom.equals("")){
				...
				JOptionPane.showMessageDialog(this.getContentPane(), pers.getNOM() + " a �t� ajouter � la base de donn�e au numero " + pers.getNUMPERSO());
				this.dispose();
			}
		}
}
